package com.example.stoxly.stockchart


import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.example.stoxly.api.MetaData
import com.example.stoxly.api.RetrofitInstance
import com.example.stoxly.api.StockData
import com.example.stoxly.api.StockResponse
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.launch

class StockViewModel : ViewModel() {
    val apiKey = "LKT1YMFIN0W0HD2B"
    ...
    fun fetchDailyData(symbol: String) = viewModelScope.launch { fetchData(symbol,
        TimeInterval.DAILY
    ) }
    fun fetchWeeklyData(symbol: String) = viewModelScope.launch { fetchData(symbol,
        TimeInterval.WEEKLY
    ) }
    fun fetchMonthlyData(symbol: String) = viewModelScope.launch { fetchData(symbol,
